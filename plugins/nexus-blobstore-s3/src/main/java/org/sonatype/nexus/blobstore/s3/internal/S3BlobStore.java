    ...
    implements BlobStore
{
  ...
  public Blob create(final InputStream blobData, final Map<String, String> headers) {
    ...
    return create(headers, destination -> {
        try (InputStream data = blobData) {
          //终于对流有了新的认识，这里限制jar包的大小为1m+data的大小，后续有需求在进行变更
          BufferedInputStream bis = new BufferedInputStream(data,104857600);
          //BufferedInputStream bis = new BufferedInputStream(data,data.available());
          MetricsInputStream input = new MetricsInputStream(bis);
          ...
          //    .waitForCompletion();
          s3.putObject(getConfiguredBucket(), destination, input, new ObjectMetadata());
  ...
  public boolean delete(final BlobId blobId, String reason) {
    ...
    final S3Blob blob = liveBlobs.getUnchecked(blobId);
    log.info("Soft deleting blob {}", blobId);
  ...
  public void init(final BlobStoreConfiguration configuration) {
    ...
    try {
      ...
      /**
       * 目前hulk上doesBucketExist方法存在问题，不论是否存在，都返回true
       */
      ...
      setConfiguredBucket(getConfiguredBucket());
    	...
    	throw new BlobStoreException("Unable to initialize blob store bucket: " + getConfiguredBucket()+" ,the cause is: " +e.getMessage(), e, null);
    }
  }
